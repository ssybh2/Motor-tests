(() => {
  ...
  function updateZoomCompensation() {
    try {
      ...
      if (!sw || !iw) return;
      const ratio = sw / iw;
      if (iw > 1800 && ratio > 0.12 && ratio < 0.74) {
        const factor = Math.min(4, Math.max(1.25, 1 / ratio));
        root.classList.add('ui-v26-zoomed');
        root.style.setProperty('--ui-v26-scale', factor.toFixed(4));
        root.style.setProperty('--ui-v26-width', `${(100 / factor).toFixed(4)}%`);
      } else {
        root.classList.remove('ui-v26-zoomed');
        root.style.removeProperty('--ui-v26-scale');
        root.style.removeProperty('--ui-v26-width');
      }
    } catch (_) {}
  }
  ...
  function normalizeMath(tex) {
    let out = String(tex ?? '');
    // App core historically used \, as multiplication spacing. Render an explicit dot product.
    out = out.replace(/\\,\s*/g, '\\cdot ');
    // Defensive repair if a literal comma was produced between coefficient and variable.
    out = out.replace(/([}\w])\s*,\s*(?=(?:\\mathrm|\\omega|\\Omega|[A-Za-z]))/g, '$1\\cdot ');
    // Human-editable K_T2 / K_Q1 etc. become proper grouped LaTeX subscripts.
    out = out.replace(/\b([A-Za-z]+)_([A-Za-z0-9]+)\b/g, '$1_{$2}');
    // Scientific notation as mathematics.
    out = out.replace(/(-?\d+(?:\.\d+)?)e([+-]?\d+)/gi, '$1\\times 10^{$2}');
    return out;
  }

  function patchKatex() {
    if (!window.katex || window.katex.__motorTestsV26Patched) return;
    ...
    window.katex.__motorTestsV26Patched = true;
  }
  ...
  function enhanceCoeffField(input) {
    if (!(input instanceof HTMLInputElement) || input.dataset.uiV26Coeff === '1') return;
    input.dataset.uiV26Coeff = '1';
    ...
    let preview = field?.querySelector('.coeff-preview-v26');
    if (label && !preview) {
      label.textContent = '系数符号 / Coefficient ';
      preview = document.createElement('span');
      preview.className = 'coeff-preview-v26';
  ...
  function refreshPowerBadges(card) {
    termRows(card).forEach((row) => {
      ...
      const badge = row.querySelector('.term-power-badge-v26');
  ...
  function makeToolButton(label, title, onClick) {
    ...
    button.type = 'button';
    button.className = 'model-tool-btn-v26';
  ...
  function enhanceTermRow(row, card) {
    if (!(row instanceof HTMLElement) || row.dataset.uiV26Term === '1') return;
    row.dataset.uiV26Term = '1';
    ...
    if (powerLabelNode && !powerLabelNode.querySelector('.term-power-badge-v26')) {
      powerLabelNode.textContent = '幂次 / Power ';
      const badge = document.createElement('span');
      badge.className = 'term-power-badge-v26';
  ...
  function enhanceModelTools(card) {
    ...
    if (!head || head.querySelector('.model-tools-v26')) return;
    const addButton = head.querySelector('.add-term');
    const tools = document.createElement('div');
    tools.className = 'model-tools-v26';
    tools.append(
  ...
  function closeAxisPickers(except = null) {
    document.querySelectorAll('.axis-select-v26.is-open').forEach((node) => {
      if (node !== except) node.classList.remove('is-open');
    });
  }

  function enhanceAxisSelect(select) {
    if (!(select instanceof HTMLSelectElement) || select.dataset.uiV26Select === '1') return;
    select.dataset.uiV26Select = '1';
    select.classList.add('native-axis-select-v26');

    const shell = document.createElement('div');
    shell.className = 'axis-select-v26';
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'axis-select-button-v26';
    button.setAttribute('aria-haspopup', 'listbox');
    button.setAttribute('aria-expanded', 'false');
    const panel = document.createElement('div');
    panel.className = 'axis-select-panel-v26';
    const search = document.createElement('input');
    search.type = 'search';
    search.className = 'axis-select-search-v26';
    search.placeholder = '搜索数据列…';
    search.autocomplete = 'off';
    const options = document.createElement('div');
    options.className = 'axis-select-options-v26';
    ...
    function rebuild(filter = '') {
      ...
      if (!all.length) {
        ...
        empty.className = 'axis-select-empty-v26';
      ...
      all.forEach((opt) => {
        ...
        item.type = 'button';
        item.className = 'axis-select-option-v26';
  ...
  function refreshAxisPickers(card = document) {
    ...
    card.querySelectorAll?.('.axis-select-v26').forEach((shell) => shell.__syncAxisSelect?.());
  }
  ...
  function markVersion() {
    document.title = 'UAV Propulsion Fit Lab · UI v2.6';
  }
  ...
  function initBodyFeatures() {
    ...
    document.addEventListener('click', (event) => {
      if (!event.target.closest('.axis-select-v26')) closeAxisPickers();
    });
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initBodyFeatures, { once: true });
  else initBodyFeatures();
})();
